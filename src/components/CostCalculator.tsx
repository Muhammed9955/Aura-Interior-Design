...
import { Send, Layers, Sliders, Info } from 'lucide-react';
...
export const CostCalculator: React.FC<CostCalculatorProps> = ({ t, lang }) => {
  ...
  const handleConfettiAndWhatsapp = () => {
    ...
    const message = `Hello Aura Interior Design, I generated a finishing cost estimate on your website:%0A- Unit Type: ${encodeURIComponent(
      ...
      tierName
    )}%0A- Estimated Average: ${avgTotal.toLocaleString()} EGP (Range: ${minTotal.toLocaleString()} - ${maxTotal.toLocaleString()} EGP)%0A%0AAvailable for a site inspection?`;

    window.open(`https://wa.me/201097855765?text=${message}`, '_blank');
  };

  return (
    <section id="calculator" className="py-24 bg-[#121110] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          {/* Controls Form */}
          <div className="lg:col-span-7 bg-[#1C1917] p-8 rounded-3xl border border-[#C5A059]/30 shadow-2xl space-y-8">
            {/* Area Slider */}
            <div>
              <div className="flex justify-between items-center mb-3">
                <label className="text-sm font-semibold text-[#F5F2EB] flex items-center gap-2">
                  <Sliders className="w-4 h-4 text-[#C5A059]" />
                  <span>{t.calculator.areaLabel}</span>
                </label>
                <span className="text-2xl font-serif font-bold text-[#F3E5AB]">m² {area}</span>
              </div>
              <input
                type="range"
                min="50"
                ...
                className="w-full h-2 bg-[#2D2823] rounded-lg appearance-none cursor-pointer accent-[#C5A059]"
              />
              <div className="flex justify-between text-[11px] text-[#A39888] mt-1">
                <span>m² 50</span>
                <span>m² 500</span>
                <span>m² 1000</span>
              </div>
            ...
            {/* Unit Type Selection */}
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-[#A39888] mb-3">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`p-3.5 rounded-2xl text-xs font-semibold border transition-all text-center cursor-pointer ${
                      unitType === item.id
                        ? 'border-[#C5A059] bg-[#2A241E] text-[#F3E5AB] shadow-md'
                ...
                {[
                  ...
                  <button
                    ...
                    className={`p-4 rounded-2xl border text-start transition-all cursor-pointer ${
                      tier === item.id
                        ? 'border-[#C5A059] bg-[#2A241E] text-[#F5F2EB] shadow-md'
                        : 'border-[#38322B] bg-[#141210] text-[#E0D5C5] hover:border-[#C5A059]/50'
                    }`}
                  >
                    <span className="block text-xs font-bold text-[#C5A059]">{item.label}</span>
                    <span className="block text-[11px] text-[#A39888] mt-1 font-mono">{item.price}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="lg:col-span-5 bg-[#1C1917] text-white p-8 rounded-3xl shadow-2xl border border-[#C5A059]/40 space-y-6 relative overflow-hidden">
            <div className="space-y-2">
              ...
              {/* Main Average Output */}
              <div className="text-3xl sm:text-4xl font-serif font-bold text-[#F3E5AB]">
                {avgTotal.toLocaleString()}{' '}
                <span className="text-base font-normal text-[#E0D5C5]">{t.calculator.egp}</span>
              </div>

              {/* Expected Range Badge */}
              <div className="pt-1">
                <span className="inline-block px-3 py-1 rounded-lg bg-[#2A241E] border border-[#C5A059]/30 text-xs text-[#E0D5C5] font-mono">
                  {lang === 'ar'
            ...
            <div className="space-y-4 pt-4 border-t border-[#38322B]">
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between text-xs mb-1 text-[#E0D5C5]">
                    <span>{t.calculator.designPlan} (10%)</span>
                    <span className="font-semibold text-white">EGP {breakdown.design.toLocaleString()}</span>
                  </div>
                  <div className="w-full h-1.5 bg-[#2D2823] rounded-full overflow-hidden">
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1 text-[#E0D5C5]">
                    ...
                    <span className="font-semibold text-white">EGP {breakdown.materials.toLocaleString()}</span>
                  </div>
                  <div className="w-full h-1.5 bg-[#2D2823] rounded-full overflow-hidden">
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1 text-[#E0D5C5]">
                    ...
                    <span className="font-semibold text-white">EGP {breakdown.supervision.toLocaleString()}</span>
                  </div>
                  <div className="w-full h-1.5 bg-[#2D2823] rounded-full overflow-hidden">
            ...
            <p className="text-[10px] text-[#A39888] italic leading-normal text-center">
              {lang === 'ar'
